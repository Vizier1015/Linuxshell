...
#install_cmd=`eval echo "$"${service_name}_install_cmd`
#build_cmd=`eval echo "$"${service_name}_build_cmd`
...
if [ $git_url"x" == "x" ];then
  ...
  echo $faild_str
  exit 1
elif [ $install_cmd"x" == "x" ];then
  echo "install_cmd is null"
  echo $faild_str
  exit 1
elif [ "$build_cmd""x" == "x" ];then
  echo "install_cmd is null"
  echo $faild_str
  exit 1
...
mv ${service_dir} ${service_name}
zip -r ${service_name}.zip ${service_name}
unzip -l ${service_name}.zip |grep index.html
if [ $? -eq 0 ];then
  echo "#############  代码压缩完成  ##############"
else
  echo "#############  代码压缩失败  ##############"
...
scp ${service_name}.zip root@$file_server:$upload_dir/
local_md5=`md5sum ${service_name}.zip |awk {'print $1'}`
remote_md5=`ssh root@$file_server md5sum $upload_dir/${service_name}.zip |awk {'print $1'}`
